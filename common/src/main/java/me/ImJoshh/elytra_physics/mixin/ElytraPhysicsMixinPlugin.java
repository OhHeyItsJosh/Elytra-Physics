...
public class ElytraPhysicsMixinPlugin implements IMixinConfigPlugin {
    ...
    public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
        ...
        if (mixinClassName.equals("me.ImJoshh.elytra_physics.mixin.PlayerPreventSnappingMixin")
                || mixinClassName.equals("me.ImJoshh.elytra_physics.mixin.PlayerInjectConstMixin"))
        {
            Boolean applySnappingFix = this.mixinFlags.get(ElytraPhysicsMixinFlags.APPLY_SNAPPING_FIX);

            if (applySnappingFix && mixinClassName.equals("me.ImJoshh.elytra_physics.mixin.PlayerPreventSnappingMixin"))
            {
                LOGGER.info("PlayerPreventSnappingMixin was chosen to be loaded!");
                return true;
            }
            else if (!applySnappingFix && mixinClassName.equals("me.ImJoshh.elytra_physics.mixin.PlayerInjectConstMixin"))
            {
                LOGGER.info("PlayerInjectConstMixin was chosen to be loaded!");
                return true;
            }
            else {
                return false;

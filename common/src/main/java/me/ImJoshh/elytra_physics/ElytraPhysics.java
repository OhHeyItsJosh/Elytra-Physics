    ...
    public static void applyMovementTransformation(PoseStack matrixStack, LivingEntity livingEntity, float partialTicks)
    {
        if (livingEntity instanceof AbstractClientPlayer abstractClientPlayerEntity) {
            if (!livingEntity.isCrouching() && !livingEntity.isFallFlying()) {
                ...
                matrixStack.mulPose(Axis.XP.rotationDegrees((leanMultiplierAdjusted * capeLean) / 2.0F + capeFlap));
                matrixStack.mulPose(Axis.YP.rotationDegrees((capeLean2 * getConfig().getRollMultiplier()) / 2.0F));
                matrixStack.mulPose(Axis.ZP.rotationDegrees((capeLean2 * getConfig().getPanMultiplier()) / 2.0F));
            }
